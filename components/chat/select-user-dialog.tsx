...
const SelectUserDialog = ({ selectedFile, onClose, onPrev }: SelectUserDialogProps) => {
  ...
  const handleSelecetedUser = (user: IUserDocument) => setSelectedUser(user);

  const handleSendMessage = async () => {
	setIsSendingMessage(true);
	try {
	  if (selectedUser && selectedUser._id) {
		await sendMessageAction(selectedUser._id.toString(), selectedFile, "image");
		router.push(`/chat/${selectedUser._id.toString()}`);
	  } else {
		// Handle the case where selectedUser or selectedUser._id is undefined
      ...
      >
        <DialogHeader>
          ...
          {users.map((user: IUserDocument) => (
            <div key={user._id.toString()} className='flex flex-col max-h-48 bg-sigSurface rounded-md overflow-auto'>
              <UserCard user={user} handleSelectedUser={handleSelecetedUser} selectedUser={selectedUser} />
            </div>
          ))}
          {isFetchingUsers && (
